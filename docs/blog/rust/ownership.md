...
```
## 借用
> 如果每个变量都只能通过转移所有权来获取一个值，那么声明了一个变量之后可以做的事情会很少，并且日常编程的灵活性也会大大降低。因此rust还有一种方式可以让别的变量获取到其他变量的值而不转移其所有权（因为所有权一旦被转移，该变量有可能会因为离开作用域提前被drop掉不可用）

rust中可以只传递一个值的引用而不转移它绑定的变量所有权，这称之为**借用**

### 什么是引用
1. 引用是一种指针类型，它指向了对象储存的内存地址
2. 引用分为可变引用和不可变引用
...
```rust
    ...
    assert_eq!(5, *s1)
```
可以看到，`s`是一个`i32`类型的整数，通过 `&` 符号对其进行一个指针创建，赋值给`s1`，此时和浅拷贝不同的是，栈中并不是存在两个 `5` 了，而是`s1`直接指向了`s`的内存地址。那么要使用`s1`所指向地址的值时该怎么办？答案是使用`*`来解引用，每一个指针类型的引用都没办法直接和值做比较的，因此用`*`来解引用获取内存地址上的值。

### 引用的限制
常规的引用如果没有特别声明，都是只读的不可变引用，但是可以通过`&mut`改为可变引用来传递。这里就会有一个需要我们思考的问题，假如一个值，它生成了若干个可变引用和不可变引用，会发生什么事呢？
```rust
fn main() {
...
```
如上所示，如果s的分裂出两个可变引用，又放出来一个不可变的只读引用，会发生什么事呢？答案就是无法预知会发生什么事～因为绑定了不可变引用的变量，它永远不知道自己接下来会解引用出什么值，而多个可变引用到处使用也会存在一个数据竞态的问题。

因此，关于引用的使用，是有以下限制的：
1. 同一个作用域内，只允许有一个可变引用。
2. 可变引用和不可变引用不能同时存在。
3. 当只有不可变引用时，可以存在多个